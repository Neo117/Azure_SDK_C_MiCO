...
#define DEFAULT_MQTT_KEEPALIVE      4*60 // 发送心跳间隔时间
#define BUILD_CONFIG_USERNAME       24
#define SAS_TOKEN_DEFAULT_LEN       10
#define RESEND_TIMEOUT_VALUE_MIN    1*10//1*60	// 发送数据超时，超过这个时间，就可以再次发送或返回
#define MAX_SEND_RECOUNT_LIMIT      2		// 在发送失败的情况下，最多可以发送的次数
...
extern volatile bool iothub_connected;
static int RetryPolicy_Linear_BackOff(bool *permit, size_t* delay, void* retryContextCallback)
{
    int result;
...
static RETRY_LOGIC* CreateRetryLogic(IOTHUB_CLIENT_RETRY_POLICY retryPolicy, size_t retryTimeoutLimitInSeconds)
{
    ...
    if (retryLogic != NULL)
    {
        ...
        case IOTHUB_CLIENT_RETRY_LINEAR_BACKOFF:
            LogError("Not implemented chosing default");
            retryLogic->fnRetryPolicy = &RetryPolicy_Linear_BackOff;//&RetryPolicy_Exponential_BackOff_With_Jitter;
            break;
